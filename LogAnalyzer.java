...
public class LogAnalyzer
{
   ...
   private static void processStartEntry(
           final String[] words, //words used in the file ex. "START session1 customer1"
           ...
           final Map<String, List<View>> viewsFromSession)
   {
      ...
      sessions.add(new View(words[VIEW_PRODUCT_ID], Integer.parseInt(words[VIEW_PRICE])));
   }
                                                 ...
                                                 final Map<String, List<Buy>> buysFromSession)
   {
      ...
      double totalViewers = 0.0;
      for(Map.Entry<String, List<View>> view: viewsFromSession.entrySet())
      {
                                                   ...
                                                   final Map<String, List<Buy>> buysFromSession)
   {
      ...
      for(Map.Entry<String, List<Buy>> buy: buysFromSession.entrySet())
      {
         System.out.println(buy.getKey());

         double totalCostViewed = 0.0;
         double totalItemsViewed = 0.0;
         for(View price: viewsFromSession.get(buy.getKey())) {
            totalCostViewed += price.getPrice();
            totalItemsViewed++;
         }

         for(Buy price: buy.getValue()) {
            System.out.println("   " + price.getProduct() + " " +
                    (price.getPrice() - (totalCostViewed / totalItemsViewed)));
         }
                                                         ...
                                                         final Map<String, List<Buy>> buysFromSession)
   {
      ...
      for(Map.Entry<String, List<String>> customer: sessionsFromCustomer.entrySet())
      {
         ...
         for(String session: customer.getValue())
         {
            if (buysFromSession.containsKey(session))
               for(Buy purchase: buysFromSession.get(session))
               {
                  int totalViews = 0;
                  for (Map.Entry<String, List<View>> views: viewsFromSession.entrySet())
                  {
                     if(customer.getValue().contains(views.getKey()))
                        for (View view: views.getValue())
                        {
                           if(view.getProduct().equals(purchase.getProduct()))
                           {
                              totalViews++;
                              break;
                           }
                  ...
                  System.out.println("   " + purchase.getProduct() + " " + totalViews);
               }
         }
      }
   ...
   private static void processFile(
      ...
      final Map<String, List<Buy>> buyFromCustomer)
   {
      while (input.hasNextLine())
      {
         processLine(input.nextLine(), sessionsFromCustomer, viewsFromCustomer, buyFromCustomer);
      }
   }
   ...
   private static void populateDataStructures(
      ...
      final Map<String, List<String>> sessionsFromCustomer,
      final Map<String, List<View>> viewsFromSession,
      final Map<String, List<Buy>> buysFromSession) throws FileNotFoundException
   {
